...
export class DocumentsService {
  ...
  for (const interaction of interactions) {
    // Quebrar pergunta e resposta em linhas também
    const questionText = `Q: ${interaction.question.replace(/\r?\n|\r/g, ' ')}`;
    const answerText = `A: ${interaction.answer.replace(/\r?\n|\r/g, ' ')}`;

    const questionLines = wrapText(questionText, fontBold, fontSize, maxTextWidth);
    const answerLines = wrapText(answerText, font, fontSize, maxTextWidth);

    // Se não couber no espaço restante da página, cria uma nova página
    const neededSpace = (questionLines.length + answerLines.length) * (fontSize + 5) + 15;
    if (currentY - neededSpace < margin) {
      pageQA = pdfDoc.addPage([pageWidth, pageHeight]);
      currentY = pageHeight - margin;
    }

    // Desenha pergunta em negrito
